  ...
  for{
    ...
    */
    for {
      ...
      funct.functionBody = append(funct.functionBody, lit)
    ...
    funct.numArgs = i
      if tok, lit := p.scanIgnoreWhitespace(); tok != CODEBRACKETCLOSE {
...
func main() {
  pars := NewParser(strings.NewReader("DEF main(asd,dsa){asss,ds,a,s,d,d,s,ss} def asdasd(){} def aaaaaa(dddsss,sdsd){asd}"))
  //pars := NewParser(strings.NewReader("DEF main(asd,dsa){asss,ds}"))
